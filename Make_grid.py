...
""" 
This script creates a grid file for ROMS idealized model runs.

Most of this script is copied and translated from previous matlab-scripts from Kjersti Daae.

The primary part unique to the setup are the creation of the slopes, the varying slope, and the
transition region. These were rudimentarily made early on in the prossess, and might be unfit 
to use as a starting point for other studies.
"""
...
from GlobParam import *                # change user outputs
...
###############################################################################
######################### - Options from "GlobParam" - ########################
###############################################################################


directory           = GlobParam.EXP_dir
grid_file_name      = GlobParam.grid_file

Descrip_grd         = GlobParam.General_desc + GlobParam.spec_desc_grd
Author              = GlobParam.Author

save_grid           = GlobParam.Create_files
...
X1                  = GlobParam.X
Y1                  = GlobParam.Y
steepnes_factor     = GlobParam.steepnes_factor
create_grid         = GlobParam.create_grid
N                   = GlobParam.z
VarSlo              = GlobParam.VarSlope
Tilted_shelf        = GlobParam.Tilted_shelf
tilt                = GlobParam.tilt
tilt_loc            = GlobParam.tilt_loc
...
if VarSlo == 0:
    ...
    transition      = GlobParam.transition
    same_top        = GlobParam.same_top
    West_slope      = GlobParam.West_slope
    East_slope      = GlobParam.East_slope
    zonal_center    = GlobParam.zonal_center
    merid_shift     = GlobParam.merid_shift
...
#Nudging and masking 
nudge_cells     = 15            # Amount of cells with nudging (away from boundary)
mask_n          = 3             # number of cells to mask at southern boundary of domain


###############################################################################
############# - Function for sloping plane (TS - Tilted shelf) - ##############
###############################################################################

def make_TS(hy):
    hy2 = np.ones([np.shape(hy)[0]])*hy[tilt_loc+55]-offs
    
    hy2[:tilt_loc] = -50                                                            # from y = 0  to y = 50
    hy2[tilt_loc:tilt_loc+50] = np.linspace(hy2[tilt_loc-1],hy2[tilt_loc+51],50)    # from y = 50 to y = 100
    hy2[tilt_loc-10:] = gaussian_filter(hy2[tilt_loc-10:],5)                        # smooth transition
    
    if tilt_loc == 83:
        hy[:tilt_loc+49] = hy2[:tilt_loc+49]
    else:
        hy[:tilt_loc+55] = hy2[:tilt_loc+55]
...
if VarSlo == 0:
    ...
    hy = offset+amplitude*((np.tanh((i*res+phase)/steepnes_factor)))  
    
    if Tilted_shelf == 1:
        if steepnes_factor == 15:
            tilt_loc = 41
        if tilt_loc == 83:
            offs = -22.3
        else:
        ...
        plt.plot(hy2[95:137],'-o',label='dummy')
        plt.plot((offset+amplitude*((np.tanh((i*res+phase)/steepnes_factor))))[95:137],label='old slope')
        plt.plot(hy[95:137],'-o',color='red',label='New tilt')
        #plt.plot([0,20],[-151,-151])
        plt.ylim([-170,-130])
        plt.legend()
    ...
    if Iplottin==1:
        ...
        ax.set_title('Full depth transect')
        ax.grid(alpha=0.3)
        ...
        cs = ax.contourf(Xcord,Ycord,h,levels=np.arange(1,1200,50),cmap='Blues',)
        cbar=fig.colorbar(cs)
        cs = ax.contour(Xcord,Ycord,h,levels=np.arange(0,1250,50),colors='gray',linewidths=.3)
    ...
    if Tilted_shelf == 1:
        ...
        hy_west = make_TS(hy_west)
        hy_east = make_TS(hy_east)
    ...
    hy_TZ = np.zeros([y.shape[0],transition])
    i     = np.arange(0,y.shape[0],1)
    ...
    ###############################################################################
    ####################### - Apply TS to transition zone - #######################
    ###############################################################################
    
    if Tilted_shelf == 1:
        hy_TZ1 = np.zeros([401,90])
        for i in range(90):
            hy_TZ1[:,i] = make_TS(hy_TZ[:,i])
        hy_TZ = hy_TZ1
    ...
    start = int(zonal_center-((transition)/2))
    stopp = int(zonal_center+((transition)/2))
    
    h=np.zeros([y.shape[0],x.shape[0]])

    h[:,:start]      = np.array([-hy_west]*start).T
    h[:,stopp:]      = np.array([-hy_east]*int(X/1000-stopp+1)).T
    h[:,start:stopp] = - hy_TZ
