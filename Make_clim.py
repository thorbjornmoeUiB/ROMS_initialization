...
if pNIJ == 1:
    if res == 3:
        ...
        if GlobalParameters.Use_stretch == 1:
            ...
            ds_u_4d[:,0:13,150:178,0:nud] = double_trouble(np.ones([13,28,nud])*-0.101)
...
if pNIIC_hydro == 1:
    z1,z2,y1,y2,vel,temp,salt = 0,30,50,95, 0.116, 3 , 35.2
    ...
    if Iplottin == 1:
        ...
        ax.set_title('Velocity at sigma lvl 5')
        X,Y = np.meshgrid(np.linspace(0,y*res,y-1),np.linspace(0,x*res,x))
        colorplot = ax.contourf(X,Y,ds_u_4d[0,5,:,:])
        cbarax = plt.colorbar(colorplot,ax=ax)
        cbarax.set_label('velocity [m/s]')
        ax.set_ylabel('distance north [km]')
        
        ax = fig.add_subplot(grid[1, 0])
        ax.set_title('Temp at sigma lvl 5')
        X,Y = np.meshgrid(np.linspace(0,y*res,y),np.linspace(0,x*res,x))
        colorplot = ax.contourf(X,Y,temp_from_init[0,5,:,:])
        cbarax = plt.colorbar(colorplot,ax=ax)
        cbarax.set_label('velocity [m/s]')
        ax.set_ylabel('distance north [km]')
        
        ax = fig.add_subplot(grid[2, 0])
        ax.set_title('Salinity at sigma lvl 5')
        X,Y = np.meshgrid(np.linspace(0,y*res,y),np.linspace(0,x*res,x))
        colorplot = ax.contourf(X,Y,salt_from_init[0,5,:,:])
        cbarax = plt.colorbar(colorplot,ax=ax)
        cbarax.set_label('velocity [m/s]')
