...
old_dir             = GlobalParameters.old_dir
...
variable_mean_file  = old_dir    + GlobalParameters.Mean_profiles
Raw_profiles        = old_dir + GlobalParameters.Raw_profiles
...
Descrip_grd         = GlobalParameters.General_desc + GlobalParameters.spec_desc_ini
Author              = GlobalParameters.Author
create_input_file   = GlobalParameters.Create_files
...
temp_3d = np.zeros([z,x,y])
salt_3d = np.zeros([z,x,y])
for i in range(y):
    temp_3d[:,:,i] = temp_2d
    salt_3d[:,:,i] = salt_2d
...
ds_u_4d    = np.zeros((1, z, x, y-1))
ds_ubar_3d = np.zeros((1, x, y-1))

#V
ds_v_3d    = np.zeros((z, x-1, y))
ds_v_4d    = np.zeros((1, z, x-1, y))
ds_vbar_3d = np.zeros((1, x-1, y))

#Z
ds_zeta_3d = np.zeros((1, x, y))
...
X = repmaty.repmat(np.linspace(0,x*res,x),30,1)
Y = - H
...
if plotting == 1:    
    ...
    plt.suptitle('FROM Make_init.py',y=1.1)
...
weird = []
for i in range(300):
    days = dt.datetime.fromordinal(int(ISt[i][0][0]))
    #print(ISt[i][0][0])
    dayfrac = dt.timedelta(days=float(ISt[i][0][0])%1) - dt.timedelta(days = 366)
    time.append(days+dayfrac)
    month.append((days+dayfrac).month)
    day.append((days+dayfrac).day/30)
    weird.append(((days+dayfrac).month + (days+dayfrac).day/30))
    print(days+dayfrac)
print(time)
plt.figure()
plt.plot(time,'.')
#%%
    ...
    plt.suptitle('FROM Make_init.py')
